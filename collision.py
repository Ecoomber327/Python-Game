...
#
#
# ****This code was NOT written by Ethan Coomber*****
#
#
#
...
def collisionTest_ball_ball(ball1, ball2):
    ...
    if length(v1) < 0.001:
        if length( [dx, dy] ) < r:
            return 0.0 # distance to impact is 0
...
def collision_ball_block(ball, block, dt, bounce=True):
    ...
    if vmag < 0.001:
        bp = ball.getPosition()
        radius = ball.getRadius()
        kp = block.getPosition()
        deltax = bp[0] - kp[0]
        deltay = bp[1] - kp[1]

        # test for collision
        if abs(deltax) < block.getWidth()/2 + radius and abs(deltay) < block.getHeight()/2 + radius:
            __last_block_side_hit = None
            return True
        else:
            return False
    ...
    # if the impact is further away
    if distToImpact > vmag * dt:
        return False
...
def collision_block_ball(block, ball, dt, bounce=True):
    return collision_ball_block(ball, block, dt, bounce)
...
collision_router[ ('ball', 'rotating line') ] = collision_ball_rotating_block

def collision(a, b, dt, bounce=True):
    return collision_router[ (a.getType(), b.getType()) ](a, b, dt, bounce)
